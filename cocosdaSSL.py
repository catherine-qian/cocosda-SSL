...
class Model2(nn.Module):
    ...
    def forward(self, x):
        ...
        # DoApred = self.pool(DoApred.transpose(1,2)).transpose(1,2).squeeze()
        # evepred = self.pool(evepred.transpose(1,2)).transpose(1,2).squeeze()
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-wts1', metavar='wts', type=float, default=0)
...
def training(epoch):
    ...
    # nb = len(train_loader)/args.batch
...
# idx1, idx2 = feat1.max(axis=2)>0, feat2.max(axis=2)>0
feat1, feat2 = minmax_norm2d(feat1, faxis=2), minmax_norm2d(feat2, faxis=2)  # attention: gccphat feature is not in channel order
